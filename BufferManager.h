...
public:
	...
	//当没有空缓存了，则用替换
	std::list<Block*>::iterator replace(std::string& fileName, int blockOffset);//使用LRU算法
	//判断缓存是否为满
	bool isFull();
	//在缓存中找某个文件的某个block。若没有，则向文件读取到缓存中。然后返回一个指向所在缓存的指针
	std::list<Block*>::iterator findBlockInBuffer(std::string fileName, int blockOffset);//得到指定的block在buffer中的iterator
	...
	void freeWithoutWrite(std::string& fileName);//由于drop表而free

	//不用某文件中的内容，则free掉缓存中关于该文件的块，需要写文件
	void free(std::string& fileName);//由于不用表而free


	void deleteRecord(std::string& fileName, int blockOffset, int start);//给recordManager
	
	//删除一个block。内部实现：将该block的内容写0
	void deleteBlock(std::string &fileName, int blockOffset);//给indexManager

	void insertRecord(std::string& fileName, std::string& content);//给recordManager
	...
	int insertBlock(std::string &fileName,  std::string& content);//给indexManager
